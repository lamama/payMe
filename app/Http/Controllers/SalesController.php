...
class SalesController extends Controller {
	...
	public function submitSale(SalesModel $salesModel,SalesCall $salesCall) {
		...
		if(isset($_GET)) {      // little validation
		...
		$params = array("seller_payme_id"=>$sellerPaymeId,"sale_price"=>$salePrice,"currency"=>$currency,"product_name"=>$productName,"installments"=>$installments,"language"=>$language);
		...
		try {
			$salesModel->storeSaleData($response);          // store the response
